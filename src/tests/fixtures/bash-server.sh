...
buildResponse(){
    ...
    # Skip content-length header to avoid curl warnings
    # The content-length calculation doesn't account for the newline added by printf '%s\n'
...
main(){
    ...
    # Try different paths for accept builtin
    accept_loaded=false
    for path in /data/data/com.termux/files/usr/lib/bash/accept /usr/lib/bash/accept accept; do
        if enable -f "$path" accept 2>/dev/null; then
            accept_loaded=true
            break
        fi
    done

    if [ "$accept_loaded" = false ]; then
        printf '%s\n' "Cannot load accept builtin from any known location..."
        exit 1
    fi
